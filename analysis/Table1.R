...
merkmal <- c("underweight", "underweight", "underweight", "normal weight", "overweight", "obesity")

# ad_sub$bmicat.0 <- factor(ad_sub$bmicat.0, labels = merkmal)
# ad_sub$bmicat.1 <- factor(ad_sub$bmicat.1, labels = merkmal)
# ad_sub$bmicat.2 <- factor(ad_sub$bmicat.2, labels = merkmal)

together <- data.frame(df1, subset(ad_sub,
                                  select = c(bmicat.0, bmicat.1, bmicat.2)))
...
table1
### save table ---------------------------------------------------------------
table1 %>% as_gt() %>% gtsave("table1_revision.rtf", path = "results/")
